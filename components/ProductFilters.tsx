...
export const ProductFilters = ({
  ...
  if (productTypeId) {
    const t = productTypes.find((x) => x.id === productTypeId);
    activeChips.push({ label: t?.name ?? "Type", onClear: () => onProductTypeChange("") });
  }
  if (stockStatus) {
    ...
    <div className="space-y-3">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
        ...
        <div className="flex shrink-0 gap-2">
          ...
          <ExportMenu basePath={exportBasePath} queryParams={exportParams} />
        </div>
      </div>

      {open ? (
        <div className="grid gap-3 rounded-xl border border-white/10 bg-[color:var(--surface)]/60 p-4 sm:grid-cols-3">
